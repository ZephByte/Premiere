...
internal object AdminCommands {
    ...
    fun movies(context: CommandContext<CommandSourceStack>): Int {
        ...
        Thread.startVirtualThread {
            val names = try {
                val keys = R2Storage.listKeys()
                val subtitleNames = keys.filter(MovieLibrary::isSubtitle)
                    .map { MovieLibrary.displayName(it).lowercase() }
                    .toSet()
                keys.filterNot(MovieLibrary::isSubtitle).map {
                    val name = MovieLibrary.displayName(it)
