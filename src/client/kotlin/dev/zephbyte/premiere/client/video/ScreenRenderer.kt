    ...
    private fun collect(context: LevelRenderContext) {
        ...
        for (active in ClientScreens.renderable()) {
            ...
            context.submitNodeCollector().submitCustomGeometry(
                ...
                // Local coords relative to origin. yc/sc center the fitted quad
                // on the wall; "left" is the viewer's left looking at the face.
                ...
                val (left, right, plane) = when (definition.facing) {
                    Direction.SOUTH -> Triple(spanLo, spanHi, 1f + EPS)
                    Direction.NORTH -> Triple(spanHi, spanLo, -EPS)
                    Direction.EAST -> Triple(spanHi, spanLo, 1f + EPS)
                    else -> Triple(spanLo, spanHi, -EPS) // WEST
                }
                val onX = definition.facing.axis == Direction.Axis.Z // wall spans X
