...
internal object ScreenCommands {
    ...
    /** sender name -> (screen name, asked-at ms) awaiting overwrite confirm. */
    private val pendingOverwrites = ConcurrentHashMap<String, Pair<String, Long>>()
    private const val OVERWRITE_CONFIRM_MS = 30_000L
    ...
    fun wand(context: CommandContext<CommandSourceStack>): Int {
        ...
        val on = SelectionTool.toggle(player)
        source.sendSuccess({
            Component.literal(
                if (on) {
    ...
    fun defineFromSelection(context: CommandContext<CommandSourceStack>): Int {
        ...
        val selection = SelectionTool.selectionOf(player)
        ...
        if (result == 1) SelectionTool.clear(player)
        return result
    }
    ...
    private fun confirmOverwrite(source: CommandSourceStack, name: String): Boolean {
        val sender = source.textName
        val pending = pendingOverwrites[sender]
        val confirmed = pending != null && pending.first == name &&
            System.currentTimeMillis() - pending.second < OVERWRITE_CONFIRM_MS
        if (!confirmed) {
            pendingOverwrites[sender] = name to System.currentTimeMillis()
            source.sendFailure(
                Component.literal("Screen '$name' already exists. Run the same command again within 30s to overwrite it.")
            )
            return false
        }
        pendingOverwrites.remove(sender)
        ScreenManager.undefine(source.server, name)
        return true
    }
    ...
    fun undefine(context: CommandContext<CommandSourceStack>): Int {
        ...
        ScreenManager.undefine(source.server, screen.definition.name)
